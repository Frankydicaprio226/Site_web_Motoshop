...
function modifier($image, $nom, $prix, $desc, $id, $quantite)
{
  if(require("connexion.php"))
  {
    $req = $access->prepare("UPDATE produits SET `image` = ?, nom = ?, prix = ?, description = ? WHERE id=?, quantite= ?");

    $req->execute(array($image, $nom, $prix, $desc, $id,$quantite));
